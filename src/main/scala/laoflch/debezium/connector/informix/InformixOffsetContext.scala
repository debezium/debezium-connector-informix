...
object InformixOffsetContext {
  ...
  class Loader(val connectorConfig: InformixConnectorConfig) extends OffsetContext.Loader {
    ...
    override def load(offset: util.Map[String, _]): OffsetContext = {
      ...
      val change_lsn = offset.asInstanceOf[util.Map[String, String]].getOrDefault(SourceInfo.CHANGE_LSN_KEY, "null").toString
      val commit_lsn = offset.asInstanceOf[util.Map[String, String]].getOrDefault(SourceInfo.COMMIT_LSN_KEY, "null").toString
      val lsn = TxLogPosition.valueOf(commit_lsn.toLong, change_lsn.toLong)
      ...
      new InformixOffsetContext(connectorConfig, lsn,
        snapshot, snapshotCompleted,
        /*eventSerialNo, */ TransactionContext.load(offset))
    }
  }
}
